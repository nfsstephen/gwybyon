...
async def get_region_colors(state: str = "", category: str = ""):
    ...
    Reads from both the default Region+territories tables AND the custom_regions table.
    When category (industry) is provided, custom_regions for that industry take priority,
    then default regions fill in uncovered counties."""
    if not state:
    ...
    try:
        ...
        # Fetch default territories for the given state that have a region_id
        ...
        # Fetch custom_regions for this state
        custom_regions_data = supabase.table("custom_regions").select("*").eq("state", state).execute()
        custom_rows = custom_regions_data.data or []

        # Build colors and region_groups
        covered_counties = set()
        colors = {}
        region_groups = {}

        # First pass: custom_regions for the selected industry (highest priority)
        if cat_id:
            for cr in custom_rows:
                if cr.get("category_id") != cat_id:
                    continue
                cr_name = cr.get("name", "")
                cr_color = cr.get("color", "#888888")
                cr_status = cr.get("status")
                cr_counties = cr.get("counties") or []
                for county in cr_counties:
                ...
                if cr_counties:
                    if cr_name not in region_groups:
                        region_groups[cr_name] = {"color": cr_color, "counties": [], "status": cr_status}
                    region_groups[cr_name]["counties"].extend([c.strip().lower() for c in cr_counties])

        # Second pass: default territories (from Region + territories tables)
        for t in t_rows:
            t_cat = t.get("category_id")
            # Skip industry-specific territory rows (old system) — only use defaults
...
async def create_territory(req: CreateTerritoryRequest):
    """Create a new custom region for a specific industry.
    Stores in custom_regions table with counties as JSON array."""
    ...
    try:
        # Pick a color not already used in this state (check both Region and custom_regions)
        existing_regions = supabase.table("Region").select("color").eq("state", state_full).execute()
        existing_custom = supabase.table("custom_regions").select("color").eq("state", state_full).execute()
        used_colors = {(r.get("color") or "").lower() for r in (existing_regions.data or []) + (existing_custom.data or [])}
        chosen_color = "#888888"
        ...
        # Build counties list as JSON
        county_names = [c.get("name", "").strip() for c in req.counties if c.get("name", "").strip()]

        # Insert into custom_regions table
        result = supabase.table("custom_regions").insert({
            "name": req.name.strip(),
            "state": state_full,
            "category_id": cat_id,
            "color": chosen_color,
        ...
        if not result.data:
            raise HTTPException(status_code=500, detail="Failed to create custom region")

        new_region = result.data[0]
        ...
        return {
            ...
            "counties": [{"county": c, "action": "created"} for c in county_names],
        }
...
async def confirm_territory(region_id: int):
    """Confirm a reserved custom region (called after deposit is paid)."""
    region = supabase.table("custom_regions").select("id, status").eq("id", region_id).execute()
    if not region.data:
        raise HTTPException(status_code=404, detail="Custom region not found")
    if region.data[0].get("status") != "reserved":
        raise HTTPException(status_code=400, detail="Region is not in reserved status")

    supabase.table("custom_regions").update({"status": "confirmed"}).eq("id", region_id).execute()
...
async def release_territory(region_id: int):
    """Release a reserved custom region — deletes it, restoring map to defaults."""
    region = supabase.table("custom_regions").select("id, name, status").eq("id", region_id).execute()
    if not region.data:
        raise HTTPException(status_code=404, detail="Custom region not found")

    region_data = region.data[0]
    if region_data.get("status") != "reserved":
        raise HTTPException(status_code=400, detail="Only reserved regions can be released")

    supabase.table("custom_regions").delete().eq("id", region_id).execute()
...
async def get_territory_pricing(req: TerritoryPricingRequest):
    ...
    # Build price lookups: category-specific first, then by TypeId only as fallback
    type_price_specific = {}
    type_price_fallback = {}
    for row in dp_rows:
        tid = row["TypeId"]
        ...
        # Always build fallback by TypeId (use highest price if multiple exist for same TypeId)
        if tid not in type_price_fallback or price > type_price_fallback[tid]:
    ...
    # Per-county: use category-specific price if available, otherwise fallback
    for county_name in req.counties:
        county_key = county_name.strip().lower()
        county_type = county_type_map.get(county_key)
        if county_type:
            price = type_price_specific.get(county_type) or type_price_fallback.get(county_type)
