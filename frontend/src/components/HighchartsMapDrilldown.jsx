...
export default function HighchartsMapDrilldown({ country, state: stateProp, selectedCounties, onToggleCounty, takenCounties, regionRefreshKey, category }) {
  ...
  const drillIntoState = useCallback(async (stateCode, stateName) => {
    ...
    const countyData = features.map(feat => {
      ...
      // Reserved counties get their color from the custom_regions table
      let displayColor = regionColor;
      if (isReserved && regionInfo) {
        displayColor = regionInfo.color; // Use the color stored in custom_regions
      }
      ...
      return {
        ...
        name: props.name || '',
        value: 1,
        color: isTaken ? '#991b1b' : isReserved ? (displayColor || '#353839') : isSelected ? '#b0f50b' : (displayColor || '#475569'),
        taken: isTaken,
        reserved: isReserved,
